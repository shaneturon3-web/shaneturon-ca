...
import { FlaskConical, Zap, ExternalLink } from 'lucide-react';
      ...
      <section className="py-24 px-4">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-sm font-mono text-primary uppercase tracking-widest mb-4">Experimental</p>
            <h1 className="font-display text-4xl sm:text-5xl font-bold tracking-tight mb-4">
              <span className="text-primary">Lab</span> / ShipYard
            </h1>
            <p className="text-lg text-muted-foreground max-w-2xl mb-16">
              Controlled experimentation. Prototypes, AI workflows, agent systems, and automation tests.
              Safely separated from the institutional layer.
            </p>
          </motion.div>
            ...
            {safe.map((project: any, i: number) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`text-xs font-mono ${statusColors[project?.status ?? 'EXPERIMENTAL'] ?? 'text-muted-foreground'}`}>
          ...
          {safe.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p>Lab experiments are in progress. Check back soon.</p>
            </div>
          )}
        </div>
      </section>
